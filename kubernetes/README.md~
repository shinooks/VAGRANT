...
# Vagrant과 Ansible을 이용한 Kubernetes 프로비저닝

## 프로젝트 개요
이 프로젝트는 Vagrant와 Ansible을 이용해 다중 VM에 Kubernetes 클러스터를 자동으로 프로비저닝하는 환경을 구성하는 방법을 다룹니다. Ansible Local Provisioner를 통해 Ansible 서버에서 Kubernetes 마스터와 워커 노드에 대한 설정을 자동화하고, 클러스터의 초기 구성을 진행합니다.
